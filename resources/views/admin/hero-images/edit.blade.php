        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form id="upload-form" method="POST" action="{{ route('admin.hero-images.update', $image) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <!-- Image Input -->
                        <div>
                            <x-input-label for="image-upload" :value="__('New Image (aspect ratio 19:6 required)')" />
                            <x-text-input id="image-upload" class="block mt-1 w-full" type="file" name="image_upload" accept="image/*" />
                            <x-input-error :messages="$errors->get('image')" class="mt-2" />
                        </div>

                        <!-- Cropped image preview -->
                        <div id="preview-container" class="mt-4" style="display: none;">
                            <x-input-label :value="__('New Cropped Image Preview')" />
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <x-primary-button id="update-button" class="ml-4">
                                {{ __('Update Image') }}
                            </x-primary-button>
                        </div>
                    </form>
